              ...
              <div className="flex justify-between items-start mb-4">
                ...
                <div className="text-right">
                  <div className="text-3xl font-bold text-[#3d3d40]">
                    SAR {auctionDetails?.highestBid ? numberWithCommas(auctionDetails?.highestBid?.amount) : numberWithCommas(car?.sellingPrice)}
                  </div>
            ...
            <div className="bg-white rounded-xl shadow-md p-6 top-24 border border-[#e9ecef]">
              <div className="mb-6">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-lg font-bold text-[#2c3e50]">SAR {auctionDetails?.highestBid ? numberWithCommas(auctionDetails?.highestBid?.amount) : numberWithCommas(car?.startPrice)}</span>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input 
                      ...
                      min={auctionDetails?.highestBid ? auctionDetails?.highestBid?.amount : car.sellingPrice} 
                      step="1000"
                      onChange={(e) => {
                        const value = parseFloat(e.target.value);
                        if (value < (auctionDetails?.highestBid ? auctionDetails?.highestBid?.amount : car.sellingPrice)) {
                          e.target.setCustomValidity(`Bid must be at least SAR ${numberWithCommas(auctionDetails?.highestBid ? auctionDetails?.highestBid?.amount : car.sellingPrice)}`);
                        } else {
                  ...
                  <p className="text-xs text-gray-500 mt-1">Minimum bid: SAR {auctionDetails?.highestBid ? numberWithCommas(auctionDetails?.highestBid?.amount) : numberWithCommas(car.sellingPrice)}</p>
                </div>
                ...
                <button 
                  ...
                  onClick={(e) => {
                    ...
                    if (!bidValue || bidValue < (auctionDetails?.highestBid ? auctionDetails?.highestBid?.amount : car.sellingPrice)) {
                      alert(`Your bid must be at least SAR ${numberWithCommas(auctionDetails?.highestBid ? auctionDetails?.highestBid?.amount : car.sellingPrice)}`);
                      return false;
                    }
